    ...
    def decode_json_string(cls, json_string: str, ignore_failed_objects: bool = False, model_directory: Path = None,
        ...
        for obj in dict_list['@graph']:
            try:
                ...
                if 'https://wegenenverkeer.data.vlaanderen.be/ns/implementatieelement#AIMObject.typeURI' not in rdf_dict:
                    rdf_dict['https://wegenenverkeer.data.vlaanderen.be/ns/implementatieelement#AIMObject.typeURI'] = \
                    rdf_dict['@type']
                del rdf_dict['@id']
                del rdf_dict['@type']
        ...
        for k, v in input_dict.items():
            ...
            if isinstance(v, dict):
                if '@value' in v and '@type' in v:
                    v = v['@value']
                elif '@id' in v and len(list(v.keys())) == 1:
                    v = v['@id']
